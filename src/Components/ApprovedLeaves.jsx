...
const ApprovedLeaves = () => {
  const [approvedLeaves, setemploye] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  useEffect(() => {
      // Fetch employe from the API
      axios.get(`${getBaseUrl()}/api/leaveAll/?status=approved`) // Example API
          .then(response => {
  ...
  return (
    <div className="table-responsive table-shadow p-6 bg-gray-100 rounded-lg shadow-lg">
      ...
      <section className="total-leaves-count mb-4">
        <p className="text-lg">
          Total approved leaves - <span className="font-bold text-gray-800">1</span>
        </p>
      </section>
